...
const History = () => {
  ...
  useEffect(() => {
    ...
    document.title = "WhiteDoorz | History";
          ...
          >
            ...
            <div className="flex items-center justify-between py-5 px-3">
              <div className="flex flex-col">
                <strong>
                  Check-In: {moment(transaction.checkIn).format("MMMM Do YYYY")}
                </strong>
                <strong>
                  Check-Out:{" "}
                  {moment(transaction.checkOut).format("MMMM Do YYYY")}
                </strong>
              </div>
              <div className="flex flex-col gap-3">
                ...
                {new Date(transaction.checkOut) < new Date() ? (
                  ...
                  <button
                    onClick={() => createPayment(transaction.id)}
                    className="py-4 text-center text-lg font-semibold disabled:bg-blue-200 disabled:cursor-not-allowed bg-blue-600 hover:bg-blue-700 text-white rounded"
                    disabled={transaction.status === "success"}
                  >
                    Bayar Sekarang
